...
// 		);
	...
	// 					// Now you can set this location to secure if needed
	// 					// For example:
	// 					secure.set('location', location);
	// 				},
	// 				error => {
	// 					console.error('Error getting location: ', error);
	// 					// In case of error, set default location
	// 					const defaultLocation = { latitude: 0, longitude: 0 };
	// 					// Set default location
	// 					// secure.set('location', defaultLocation);
	...
	// 		// If permission denied, set default location
	// 		const defaultLocation = { latitude: 0, longitude: 0 };
	// 		// Set default location
	// 		secure.set('location', defaultLocation);
...
const useGlobal = create((set, get) => ({
	...
	init: async () => {
		...
		if (credentials) {
			try {
				...
				// Set user's location in the store
	...
	socketConnect: async () => {
		...
		const url = `ws://192.168.213.184:8000/chat/?token=${tokens.access}`
		console.log(url);
	...
	firstmessagesend: (recipient_username, message) => {
		const socket = get().socket
		socket.send(JSON.stringify({
			source: 'firstmessage',
	...
	worker: false, // New field for worker status

	toggleWorker: () => {
		// console.log(worker);
		set((state) => ({ worker: !state.worker }));
	},
	...
	uploadThumbnail: async (file) => {
		...
		if (!get().socket) {
			// Call socketConnect if socket is not initialized
			await get().socketConnect();
		}

		// Get the socket after ensuring it's initialized
		const socket = get().socket;

		// Check if socket is still not initialized
		if (!socket) {
			console.error("Socket is not initialized. Thumbnail upload failed.");
			return;
		}
